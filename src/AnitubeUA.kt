...
class AnitubeUA : AnimeSource {
    ...
    override fun getAnimeList(): List<Anime> {
        val html = client.newCall(okhttp3.Request.Builder().url("$baseUrl/anime-list").build())
    ...
    override fun getEpisodes(anime: Anime): List<Episode> {
        val html = client.newCall(okhttp3.Request.Builder().url(anime.url).build())
            .execute().body?.string() ?: return emptyList()
        val doc = Jsoup.parse(html)
    ...
    override fun getVideos(episode: Episode): List<Video> {
        val html = client.newCall(okhttp3.Request.Builder().url(episode.url).build())
            .execute().body?.string() ?: return emptyList()
        val doc = Jsoup.parse(html)
